...
class CallEntry extends Model {
	...
	{
		return
			$query->join('equipments','call_entries.equip_id','=','equipments.id')->whereIn('equipments.healthfacility_id', auth()->user()->healthfacilitys->pluck('id')->toArray())->select('call_entries.*');
	}
