...
class PreventiveController extends Controller {
	...
	public function index() {
		...
		$index['p_maintenance'] = CallEntry::where('call_type', 'preventive')->latest()->healthfacility()->get();
		$index['users'] = User::pluck('name', 'id');
		return view('call_preventive.index', $index);
	}
	public function export($type)
	{
		$preventive = CallEntry::where('call_type', 'preventive')->latest()->healthfacility()->get();
	...
	public function create() {
		...
		$index['page'] = 'preventive_maintenance';
		$index['unique_ids'] = Equipment::query()->healthfacility()->pluck('unique_id', 'id')->toArray();
		$index['departments'] = Department::select('id', \DB::raw('CONCAT(short_name,"(",name,")") as department'))->pluck('department', 'id')->toArray();
		$index['healthfacilitys'] = healthfacility::query()->healthfacility()->pluck('name', 'id')->toArray();
	...
	public function edit($id) {
		...
		$index['unique_ids'] = Equipment::query()->healthfacility()->withTrashed()->where('healthfacility_id', $index['preventive']->equipment->healthfacility_id)
			->pluck('unique_id', 'id')
			->toArray();
		$h_id = $index['preventive']->equipment->healthfacility_id;
		$index['departments'] = Department::select('id', \DB::raw('CONCAT(short_name,"(",name,")") as department'))
			->whereHas('equipments', function ($q) use ($h_id) {
				$q->where('healthfacility_id', $h_id);
			})
			->pluck('department', 'id')
			->toArray();

		$index['healthfacilitys'] = healthfacility::withTrashed()->healthfacility()->pluck('name', 'id')->toArray();
	...
	public function ajax_healthfacility_change(Request $request) {
		if ($request->ajax()) {
			$unique_id = Equipment::where('healthfacility_id', $request->id)
				->pluck('unique_id', 'id')
				->toArray();

			$department = Equipment::where('healthfacility_id', $request->id)
	...
	public function ajax_department_change(Request $request) {
		if ($request->ajax()) {
			if ($request->healthfacility_id && $request->healthfacility_id != "") {

				$unique_id = Equipment::where('department', $request->department)
					->where('healthfacility_id', $request->healthfacility_id)
					->pluck('unique_id', 'id')
					->toArray();
			} else {
				$unique_id = Equipment::where('department', $request->department)
