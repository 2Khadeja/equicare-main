...
class ReportController extends Controller {
	...
	public function time_indicator() {
		...
		$index['healthfacilitys'] = healthfacility::query()->healthfacility()->pluck('name', 'id');
		$index['equipments'] = Equipment::query()->healthfacility()->pluck('unique_id', 'id');

		// $index['call_entries'] = CallEntry::leftJoin('equipments', 'call_entries.equip_id', '=', 'equipments.id')
		// 	->leftJoin('healthfacilitys', 'equipments.healthfacility_id', '=', 'healthfacilitys.id')
		// 	->select('*')
		// 	->paginate(10);
		$index['call_entries'] = CallEntry::query()->healthfacility()
			->paginate(10);
	...
	public function time_indicator_filter(Request $request) {
		...
		$index['healthfacilitys'] = healthfacility::query()->healthfacility()->pluck('name', 'id');
		$index['equipments'] = Equipment::query()->healthfacility()->pluck('unique_id', 'id');
		$index['healthfacility'] = $request->healthfacility;
		...
		$call_entries = CallEntry::query()->healthfacility();
		if (isset($request->healthfacility) && $request->healthfacility != "") {
			$call_entries->where('equipments.healthfacility_id', $request->healthfacility);
		}
	...
	public function ajax_to_get_equipment(Request $request) {
		if ($request->ajax()) {
			if ($request->healthfacility_id) {
				$equipments_filter = Equipment::where('healthfacility_id', $request->healthfacility_id)
					->pluck('unique_id', 'id');
			} else {
				$equipments_filter = Equipment::query()->healthfacility()->pluck('unique_id', 'id');
			}
		}
	...
	public function equipment_report() {
		...
		$index['healthfacilitys'] = healthfacility::query()->healthfacility()->pluck('name', 'id');
		$index['call_entries'] = CallEntry::query()->healthfacility()->paginate(10);
	...
	public function equipment_report_post(Request $request) {
		...
		if (isset($request->healthfacility)) {
			$call_entries->whereHas('equipment', function ($q) use ($request) {
				$q->where('healthfacility_id', $request->healthfacility);
			});
		}
		if (isset($request->excel_hidden)) {
		    //   dd($call_entries->get());
