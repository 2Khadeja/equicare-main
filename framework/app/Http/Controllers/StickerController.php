...
class StickerController extends Controller
{
    ...
    "), function ($join) {
			...
			->healthfacility() // Apply the healthfacility scope here
			->paginate(10);
	...
	public function post(Request $request)
	{
		...
		if ($request->action == "Generate Stickers") {
			...
			$index['calibrations'] = $calibrations->groupBy('calibrations.equip_id')->paginate(10)->appends(request()->only('healthfacility', 'unique_id'));
	...
	{
		$equipments = Equipment::where('healthfacility_id', $request->healthfacility_id)
