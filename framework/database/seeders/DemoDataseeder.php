...
class DemoDataseeder extends Seeder {
	...
	public function run() {
		...
		$healthfacility_suffix = ['healthfacility', 'Medical', 'Care', 'Medicare', 'Dental', 'Clinic', 'Trust', 'Org'];

		// 15 duplicate records
		for ($i = 1; $i <= 5; $i++) {

			/**
			 * Dummy healthfacility Create code below
			 */
			$healthfacility = healthfacility::create([
				'name' => $faker->company . " " . $healthfacility_suffix[array_rand($healthfacility_suffix)],
			...
			$healthfacility->save();
			$user = User::find(1);
			$user->healthfacilitys()->sync([1,2,3,4,5]);
			// $user->save();
			...
			$equipment_number = Equipment::where('healthfacility_id', $equipment->healthfacility_id)
			...
			$equipment->unique_id = "";
			$healthfacility = healthfacility::where('id', $equipment->healthfacility_id)->first();
			if ($healthfacility != "")
			// {
	...
	public static function recursive($yourString, $table_name = "") {
		...
		if (strpos($yourString, " ") === false) {
			...
			if (array_key_exists(2, $words) && $table_name != 'healthfacility') {
				...
				for ($i = 1; $i <= strlen($yourString); $i++) {
					...
					if ($table_name == 'healthfacility') {
						...
						if (is_null($check_first_two_char)) {
							...
							self::recursive($yourString, 'healthfacility');
						}
					} else {
