				...
				<div class="box-body">
					...
					<div class="row">
						<div class="form-group col-md-3">
							{!! Form::label('healthfacility',__('equicare.healthfacility')) !!}
							{!! Form::select('healthfacility',$healthfacilitys??[],$healthfacility??null,['class'=>'form-control select2_healthfacility','placeholder'=>__('equicare.select')]) !!}
						</div>
						<div class="form-group col-md-3">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						...
						<tbody>
							...
							<tr>
								<td> {{ $count }} </td>
								<td>{{ $calibration->equipment?$calibration->equipment->healthfacility->name : '-' }}
								</td>
...
@section('scripts')
	...
	<script type="text/javascript">
		$(function() {
			...
			$('.select2_equipment, .select2_healthfacility').select2({
				allowClear:true,
				placeholder:"{{__("equicare.select_option")}}"
			});
			$('select[name=healthfacility]').change(function(){
				var healthfacility_id = $(this).val();
				if(healthfacility_id){
				$.ajax({
