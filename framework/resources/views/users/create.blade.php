                ...
                <div class="box-body ">
                    ...
                    <form class="form" method="post" action="{{ route('users.store') }}">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                <label for="healthfacilitys"> @lang('equicare.healthfacilitys') <span class="select_all_msg"></span> </label>
                                <select class="select2_healthfacility form-control" multiple name="healthfacilitys[]">
                                    <option value="selectAll" @if (old('healthfacilitys') && in_array('selectAll', old('healthfacilitys'))) selected @endif>
                                        Select All
                                    </option>
                                    @foreach ($healthfacilitys as $healthfacility)
                                        <option value="{{ $healthfacility->id }}"
                                            @if (old('healthfacilitys') && in_array($healthfacility->id, old('healthfacilitys'))) selected @endif>
                                            {{ $healthfacility->name ?? '' }}
                                        </option>
                                    @endforeach
                                </select>
...
@section('scripts')
    <script>
        $('.select2_healthfacility').select2({
            allowClear: true,
            placeholder: "@lang('equicare.select_healthfacility')"
        });
        $('.select2_role').select2({
            allowClear: true,
            placeholder: "@lang('equicare.select_role')"
        });
        $('.select2_healthfacility').on('select2:select', function() {
            if ($(this).val() !== null && $(this).val().includes('selectAll')) {
                // Select all options
                $('#select_all').val(1);
                $('.select_all_msg').text('(Select All option automatically assigns a new healthfacility to a user.)');
                $(this).val($(this).find('option').not(':first').map(function() {
                    return this.value;
                })).trigger('change');
            }
        });
				 $('.select2_healthfacility').on('select2:unselect', function() {
								$('#select_all').val(0);
                $('.select_all_msg').text('');
				 });
    </script>
