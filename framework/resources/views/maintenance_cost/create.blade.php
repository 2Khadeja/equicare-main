                ...
                <div class="box-body ">
                    ...
                    <div class="row">
                        <div class="form-group col-md-4">
                            {!! Form::label('healthfacility_id', __('equicare.healthfacility')) !!}
                            {!! Form::select('healthfacility_id', $healthfacilitys ?? [], null, [
                                'class' => $errors->has('healthfacility_id')
                                    ? 'is-invalid form-control select2_healthfacility'
                                    : 'form-control select2_healthfacility',
                                'placeholder' => 'Select healthfacility',
                            ]) !!}
                            @if ($errors->has('healthfacility_id'))
                                <strong class="invalid-feedback">
                                    <span>{{ $errors->first('healthfacility_id') }}
                                    </span>
                                </strong>
                            @endif
                        </div>
...
@section('scripts')
    ...
    function equipment_old_select(){
        ...
        $(document).ready(function() {
            ...
            $('.select2_healthfacility').select2({
            ...
            $('select[name=healthfacility_id]').on('change', function() {
                var healthfacility_id = $(this).val();
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.equipments.length == 0) {
                            alert('{{ __('equicare.select_other_healthfacility') }}');
                        } else {
					...
					$('#fa-fa-plus-id').click(function() { 
						console.log('hi');
						var healthfacility_id = $('select[name=healthfacility_id]').val();
						console.log(healthfacility_id);
					$.ajax({
						url: "{{ url('get_equipment') }}",
						type: 'get',
						data: {
        ...
        function loadEquipAjax() {
            if ($('select[name=healthfacility_id]').val()) {
                $('select[name=healthfacility_id]').trigger('change');
            }
        }
    </script>
    <script type="text/javascript">
